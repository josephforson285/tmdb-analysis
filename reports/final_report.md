...
**What one row is.** The unit of observation is a single movie. 
The project brief
...
|---|---|---|
| Extraction | `01_data_extraction` | `data/raw/movies_raw.json` (raw API responses) |
...
- **Zeros treated as missing.** A budget/revenue/runtime of 0 means "unknown", not "free" — if
...
| Most popular  | Avengers: Infinity War | 36.2 |
...
Budgets are around 125–360 M$ while revenue spans 1.2–2.9 B$. Within
this  sample, budget is a weak predictor: Star Wars: The Last Jedi spent $300M for
...
The well-represented genres (like Adventure n=15, Action and Science Fiction n=10) has the median
ROI in the range of 7–8. 
...
TMDB popularity shows a moderate positive correlation with rating (r = 0.66, n = 18). Popularity is
TMDB's internal unitless score which can change based on engagement.  It ranks current platform buzz, and  moves slightly with 
ratings. Endgame is tied for the best rating in the sample (8.24) yet has barely half the
buzz of the identically-rated Infinity War  and The Avengers). 
### Yearly box office

![Yearly revenue](figures/04_yearly_revenue.png)

2015 is the sample's peak year ($6.7B across 4 movies), with 2018–2019 close behind. The gaps (1998–2008) are gaps in the sample, not in cinema history.

### Franchise vs. standalone

![Franchise vs standalone](figures/05_franchise_vs_standalone.png)

The two standalones  beats the 16 franchise movies on
...
2. **Tiny groups.** The data is not large enough when it comes to groups. Standalone has n=2 and several genres n≤2. 

## 4. Conclusion

A four-stage, tested, reproducible pipeline turned raw nested API JSON into validated
KPIs and honest visualizations. Technical and analytical actions drove this walkthrough in terms of 
